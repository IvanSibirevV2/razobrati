...
namespace FuzzyLibrary {
  ...
  public interface IMembershipFunctionExt : IMembershipFunction {
    double GetLeftBoundX();
    double GetRightBoundX();
    double GetMaxX();
    List<double> GetCrisp(double mf);
  }

  /// <summary>
  /// Triangular membership function
  /// </summary>
  public class TriangularMembershipFunction :IMembershipFunctionExt {
    double _x1, _x2, _x3;
    ...
    public List<double> GetCrisp(double mf) {
      var crisps = new List<double>();
      var c1 = (X2 - X1) * mf + X1;
      var c2 = (X2 - X3) * mf + X3;
      crisps.Add(c1);
      crisps.Add(c2);
  ...
  /// </summary>
  public class TrapezoidMembershipFunction :IMembershipFunctionExt {
  ...
  public class ConstantMembershipFunction : IMembershipFunction {
    ...
    public ConstantMembershipFunction(double constValue) {
        constValue = Math.Abs(constValue - 1.0) < Math.Pow(1, -5) ? 1.0 : constValue;
  ...
  /// </summary>
  public class PointwiseMembershipFunction : IMembershipFunction {
    
    public SPointList PointList;

    public PointwiseMembershipFunction(IEnumerable<SPoint> pointList) {
      PointList = new SPointList();
      foreach (SPoint point in pointList) {
    ...
    /// <returns></returns>
    public double GetValue(double x) {
      return PointList.InterpolateX(x);
    }
  ...
  public class CompositeMembershipFunction : IMembershipFunction {
    readonly List<IMembershipFunction> _mfs = new List<IMembershipFunction>();
