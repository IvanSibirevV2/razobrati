...
namespace QWE_2015_003{
    public partial class Clustering{
        public static class Ward{
            private static Double TTS(List<List<List<string>>> _LLS)
            {
                Double SUMM = 0;
            ...
            private static Double ESS(List<List<List<string>>> _LLS)
            {
                Double SUMM = 0;
                for (int k = 1; k < _LLS[0][0].Count(); k++){
                    for (int i = 0; i < _LLS.Count(); i++){
            ...
            private static Double r(List<List<List<string>>> __LLS, int _min_i, int _min_j, int _n)
            {
                ...
                _LLS.Add(newLLS);
                return Math.Pow((TTS(_LLS) - ESS(_LLS)) / TTS(_LLS), 2); 
            }            
            ...
            public static List<List<List<string>>> GO_v0(List<List<string>> LLS, int n, Log ThisLog)
            {
                ThisLog = ThisLog.NewLog("C.W.G0", "Clustering.Ward.GO_v0", true);
                ...
                if (n >= 2) 
                    while (n < ClusterList.Count())
                    {
                        ...
                        string progress = ThisLog.FullName + " : " + Convert.ToString(ClusterList.Count() - 1) + "/" + Convert.ToString(n);
