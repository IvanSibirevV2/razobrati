...
namespace QWE_2015_003{
    public partial class Clustering{
        public static class Centroid{/// <summary>C.Comment("CCM - кластеризация центроидным методом_ClusteringCentroidsMethod"); (классика жанра, как есть, без костылей стимуляторов и ускорителей) </summary>
            public static List<List<List<string>>> GO_v1(List<List<string>> LLS, int n, Log ThisLog)
            {
                ThisLog = ThisLog.NewLog("Clu.C.G1", "Clustering.Centroid.GO_v1", true);
            ...
            public static List<List<List<string>>> GO_Viper(List<List<string>> LLS, int n, Log ThisLog)
            {                
                ThisLog = ThisLog.NewLog("Clu.C.G_V", "Clustering.Centroid.GO_Viper to "+ Convert.ToString(n), true);
