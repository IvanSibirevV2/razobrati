...
namespace QWE_2015_003
{
    public partial class PreProInpDat
    {
        ...
        static public class Replenishment
        {
            ...
            public static List<List<string>> PrimaryReplenishmentNaNElements(List<List<string>> LLS, Log ThisLog)
            {
                ThisLog = ThisLog.NewLog("PPID.R.PRNaNE", "PreProInpDat.Replenishment.PrimaryReplenishmentNaNElements", true);
            ...
            public static List<List<string>> PrimaryReplenishmentNaNElementsWithZeros(List<List<string>> LLS){
                for (int j = 1; j < LLS[0].Count(); j++)
                    for (int i = 1; i < LLS.Count(); i++)
                        if (LLS[i][j] == "NaN")
